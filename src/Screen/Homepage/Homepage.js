...
const Homepage = (props) => {
  ...
  const getDataFilm = async () => {
    try {
      const listFilm = await axios.get(
        `https://movieapp-glints.herokuapp.com/api/v1/movies/page/1`
      );
  ...
  const getListGenre = async () => {
    try {
      const genres = await axios.get(
        "https://movieapp-glints.herokuapp.com/api/v1/categories/"
      );
  ...
  const updateSearch = (search) => {
    setSearch(search);
    const searchFilm = dataFilm.filter((film) =>
  ...
  const MoveToDetail = async (e) => {
    ...
    try {
      const res = await axios.get(
        `https://movieapp-glints.herokuapp.com/api/v1/movies/${e.id}`
      );
  ...
  const MoveToAllReviews = async (e) => {
    try {
      const res = await axios.get(
        `https://movieapp-glints.herokuapp.com/api/v1/reviews/movie/${e.id}/7`
      );
            ...
            <ScrollView horizontal style={styles.genres}>
              {listGenre.map((e, i) => {
                const getFilmPerGenre = async () => {
                  try {
                    const filmPerGenre = await axios.get(
                      `https://movieapp-glints.herokuapp.com/api/v1/movies/${e.name}/page/1`
                    );
                      ...
                      <Image
                        source={
                          activeGenre === i
                            ...
                            : require("../../Assets/Image/Icon(1).png")
                        }
                        style={styles.icon}
                      />
                      <Text
                        style={[
                          { color: activeGenre === i ? "white" : "black" },
                          styles.genresText,
                        ]}
                      >
                        {e.name}
              ...
              <TouchableOpacity onPress={() => MoveToDetail(e)}>
                <Card containerStyle={styles.card}>
                  ...
                  <View style={styles.reviewShare}>
                    <Button
                      type="clear"
                      icon={<Icon name="message-circle" type="feather" />}
                      titleStyle={{ color: "black" }}
                      onPress={() => MoveToAllReviews(e)}
                    ></Button>
                    <Button
                      type="clear"
                      icon={<Icon name="share" type="foundation" />}
                    ></Button>
                  </View>
                </Card>
              </TouchableOpacity>
